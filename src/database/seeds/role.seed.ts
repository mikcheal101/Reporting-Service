...
export class RoleSeed {
  static run = async (applicationContext: INestApplicationContext) => {
    const roleRepository = applicationContext.get<Repository<Role>>(
    ...
    const permissionRepository = applicationContext.get<Repository<Permission>>(
      getRepositoryToken(Permission),
    );
    const permissions: Array<Permission> = await permissionRepository.find();

    superAdmin.permissions = Array.from(
      new Set([...(superAdmin.permissions || []), ...permissions]),
    );
