...
export class MariaDBAdapter implements IDatabaseAdapter {
  ...
  constructor(connectionRequestDto: ConnectionRequestDto) {
    ...
    this.logger = new Logger(MariaDBAdapter.name);
  }
  ...
  public queryAsync = async (
    ...
    try {
      ...
      if (error?.code === 'ETIMEDOUT' || error?.errno === 3024) {
        this.logger.warn(`MariaDB timed out after ${timeOutMs} ms.`);
        throw new DatabaseTimeOutError(timeOutMs);
      }

      if (error?.errno === 1213) {
        this.logger.warn(`MariaDB Deadlock detected!`);
        throw new DatabaseDeadLockError(DatabaseType.MariaDB);
      }
