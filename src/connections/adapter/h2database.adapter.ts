...
export class H2DatabaseAdapter implements IDatabaseAdapter {
  private pool: Pool;
  private client: any;
  ...
  constructor(connectionRequestDto: ConnectionRequestDto) {
    ...
    this.logger = new Logger(H2DatabaseAdapter.name);
  }
  ...
  public connectAsync = async (): Promise<boolean> => {
    try {
      ...
      this.logger.log('connected to h2 database (pg-compat mode)!');
      return true;
    } catch (error) {
  ...
  public queryAsync = async (
    ...
    try {
      ...
      if (error?.code === 'ETIMEDOUT' || error?.code === '57014') {
        this.logger.warn(`H2Database timed out after ${timeOutMs} ms.`);
        throw new DatabaseTimeOutError(timeOutMs);
      }

      if (error?.code === '40001') {
        this.logger.warn(`H2Database Deadlock detected!`);
        throw new DatabaseDeadLockError(DatabaseType.H2Database);
      }
